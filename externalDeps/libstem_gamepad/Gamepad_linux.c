  ...
  if (inited) {
    ...
    do {
      ...
      if (devicesLeft > 0) {
        ...
        thread =
            ((struct Gamepad_devicePrivate *)devices[0]->privateData)->thread;
    ...
    for (eventIndex = 0; eventIndex < eventCount; eventIndex++) {
      ...
      } else if (eventQueue[eventIndex].eventType ==
...
static void queueEvent(unsigned int deviceID, enum Gamepad_eventType eventType,
                       void *eventData) {
  ...
  if (eventCount >= eventQueueSize) {
    ...
    eventQueue = realloc(eventQueue,
                         sizeof(struct Gamepad_queuedEvent) * eventQueueSize);
  }
...
static void *deviceThread(void *context) {
  ...
  while (read(devicePrivate->fd, &event, sizeof(struct input_event)) > 0) {
    if (event.type == EV_ABS) {
      ...
      value = (event.value - devicePrivate->axisInfo[event.code].minimum) /
                  (float)(devicePrivate->axisInfo[event.code].maximum -
                          devicePrivate->axisInfo[event.code].minimum) *
                  2.0f -
              1.0f;
      queueAxisEvent(device, event.time.tv_sec + event.time.tv_usec * 0.000001,
                     devicePrivate->axisMap[event.code], value,
      ...
      if (event.code < BTN_MISC || event.code > KEY_MAX ||
          devicePrivate->buttonMap[event.code - BTN_MISC] == -1) {
        continue;
      }

      queueButtonEvent(
          device, event.time.tv_sec + event.time.tv_usec * 0.000001,
          devicePrivate->buttonMap[event.code - BTN_MISC], !!event.value);

      device->buttonStates[devicePrivate->buttonMap[event.code - BTN_MISC]] =
          !!event.value;
    }
  }
  ...
  for (gamepadIndex = 0; gamepadIndex < numDevices; gamepadIndex++) {
    if (devices[gamepadIndex] == device) {
      ...
      numDevices--;
      for (gamepadIndex2 = gamepadIndex; gamepadIndex2 < numDevices;
           gamepadIndex2++) {
...
void Gamepad_detectDevices() {
  ...
  if (dev_input != NULL) {
    while ((entity = readdir(dev_input)) != NULL) {
      charsConsumed = 0;
      if (sscanf(entity->d_name, "event%d%n", &num, &charsConsumed) &&
          charsConsumed == strlen(entity->d_name)) {
        ...
        duplicate = false;
        for (gamepadIndex = 0; gamepadIndex < numDevices; gamepadIndex++) {
          if (!strcmp(((struct Gamepad_devicePrivate *)devices[gamepadIndex]
                           ->privateData)
                          ->path,
                      fileName)) {
            duplicate = true;
        ...
        if (!test_bit(EV_KEY, evCapBits) || !test_bit(EV_ABS, evCapBits) ||
            ...
            (!test_bit(BTN_TRIGGER, evKeyBits) && !test_bit(BTN_A, evKeyBits) &&
             !test_bit(BTN_1, evKeyBits))) {
          close(fd);
          continue;
        }
        ...
        devices = realloc(devices,
                          sizeof(struct Gamepad_device *) * (numDevices + 1));
        ...
        memset(deviceRecordPrivate->buttonMap, 0xFF,
               sizeof(deviceRecordPrivate->buttonMap));
        memset(deviceRecordPrivate->axisMap, 0xFF,
               sizeof(deviceRecordPrivate->axisMap));
        ...
        for (bit = 0; bit < ABS_CNT; bit++) {
          if (test_bit(bit, evAbsBits)) {
            if (ioctl(fd, EVIOCGABS(bit), &deviceRecordPrivate->axisInfo[bit]) <
                    0 ||
                deviceRecordPrivate->axisInfo[bit].minimum ==
                    deviceRecordPrivate->axisInfo[bit].maximum) {
              continue;
            }
        ...
        for (bit = BTN_MISC; bit < KEY_CNT; bit++) {
          if (test_bit(bit, evKeyBits)) {
            deviceRecordPrivate->buttonMap[bit - BTN_MISC] =
        ...
        pthread_create(&deviceRecordPrivate->thread, NULL, deviceThread,
                       deviceRecord);
      }
    }
    closedir(dev_input);
