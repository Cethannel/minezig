/*
  ...
  Some modifications have been done to this file
*/
...
static void queueInputEvent(unsigned int deviceID,
                            enum Gamepad_eventType eventType, void *eventData) {
  ...
  if (inputEventCount >= inputEventQueueSize) {
    inputEventQueueSize =
        inputEventQueueSize == 0 ? 1 : inputEventQueueSize * 2;
    inputEventQueue =
        realloc(inputEventQueue,
                sizeof(struct Gamepad_queuedEvent) * inputEventQueueSize);
  }
  inputEventQueue[inputEventCount++] = queuedEvent;
}

static void queueAxisEvent(struct Gamepad_device *device, double timestamp,
                           unsigned int axisID, float value, float lastValue) {
...
static void queueButtonEvent(struct Gamepad_device *device, double timestamp,
                             unsigned int buttonID, bool down) {
  ...
  queueInputEvent(device->deviceID,
                  down ? GAMEPAD_EVENT_BUTTON_DOWN : GAMEPAD_EVENT_BUTTON_UP,
                  buttonEvent);
}

static void onDeviceValueChanged(void *context, IOReturn result, void *sender,
                                 IOHIDValueRef value) {
  ...
  for (axisIndex = 0; axisIndex < deviceRecord->numAxes; axisIndex++) {
    if (!hidDeviceRecord->axisElements[axisIndex].isHatSwitchSecondAxis &&
      ...
      if (IOHIDValueGetLength(value) > 4) {
        // Workaround for a strange crash that occurs with PS3 controller; was
        // getting lengths of 39 (!)
        continue;
      }
      ...
      if (hidDeviceRecord->axisElements[axisIndex].isHatSwitch) {
        ...
        if (!hidDeviceRecord->axisElements[axisIndex].hasNullState) {
          if (integerValue <
              hidDeviceRecord->axisElements[axisIndex].logicalMin) {
            integerValue = hidDeviceRecord->axisElements[axisIndex].logicalMax -
                           hidDeviceRecord->axisElements[axisIndex].logicalMin +
                           1;
          } else {
            integerValue--;
        ...
        hatValueToXY(integerValue,
                     hidDeviceRecord->axisElements[axisIndex].logicalMax -
                         hidDeviceRecord->axisElements[axisIndex].logicalMin +
                         1,
                     &x, &y);
        ...
        if (x != deviceRecord->axisStates[axisIndex]) {
          queueAxisEvent(deviceRecord,
                         IOHIDValueGetTimeStamp(value) * timebaseInfo.numer /
                             timebaseInfo.denom * 0.000000001,
                         axisIndex, x, deviceRecord->axisStates[axisIndex]);
        ...
        floatValue =
            (integerValue -
             hidDeviceRecord->axisElements[axisIndex].logicalMin) /
                (float)(hidDeviceRecord->axisElements[axisIndex].logicalMax -
                        hidDeviceRecord->axisElements[axisIndex].logicalMin) *
                2.0f -
            1.0f;
...
static int IOHIDDeviceGetIntProperty(IOHIDDeviceRef deviceRef,
                                     CFStringRef key) {
  CFTypeRef typeRef;
  int value;
...
static void onDeviceMatched(void *context, IOReturn result, void *sender,
  ...
  devices =
      realloc(devices, sizeof(struct Gamepad_device *) * (numDevices + 1));
  ...
  if (cfProductName == NULL ||
      CFGetTypeID(cfProductName) != CFStringGetTypeID()) {
    ...
    CFStringGetBytes(cfProductName,
                     CFRangeMake(0, CFStringGetLength(cfProductName)),
                     kCFStringEncodingUTF8, '?', false, NULL, 100, &length);
    description = malloc(length + 1);
    CFStringGetBytes(cfProductName,
                     CFRangeMake(0, CFStringGetLength(cfProductName)),
                     kCFStringEncodingUTF8, '?', false, (UInt8 *)description,
                     length + 1, NULL);
  ...
  for (elementIndex = 0; elementIndex < CFArrayGetCount(elements);
    ...
    // All of the axis elements I've ever detected have been
    // kIOHIDElementTypeInput_Misc. kIOHIDElementTypeInput_Axis is only included
    // for good faith...
    if (type == kIOHIDElementTypeInput_Misc ||
        type == kIOHIDElementTypeInput_Axis) {

      hidDeviceRecord->axisElements =
          realloc(hidDeviceRecord->axisElements,
                  sizeof(struct HIDGamepadAxis) * (deviceRecord->numAxes + 1));
      hidDeviceRecord->axisElements[deviceRecord->numAxes].cookie =
          IOHIDElementGetCookie(element);
      hidDeviceRecord->axisElements[deviceRecord->numAxes].logicalMin =
          IOHIDElementGetLogicalMin(element);
      hidDeviceRecord->axisElements[deviceRecord->numAxes].logicalMax =
          IOHIDElementGetLogicalMax(element);
      hidDeviceRecord->axisElements[deviceRecord->numAxes].hasNullState =
          !!IOHIDElementHasNullState(element);
      hidDeviceRecord->axisElements[deviceRecord->numAxes].isHatSwitch =
          IOHIDElementGetUsage(element) == kHIDUsage_GD_Hatswitch;
      hidDeviceRecord->axisElements[deviceRecord->numAxes]
          .isHatSwitchSecondAxis = false;
      deviceRecord->numAxes++;

      if (hidDeviceRecord->axisElements[deviceRecord->numAxes - 1]
              .isHatSwitch) {
        hidDeviceRecord->axisElements = realloc(
            hidDeviceRecord->axisElements,
            sizeof(struct HIDGamepadAxis) * (deviceRecord->numAxes + 1));
        hidDeviceRecord->axisElements[deviceRecord->numAxes]
            .isHatSwitchSecondAxis = true;
        deviceRecord->numAxes++;
      }

    } else if (type == kIOHIDElementTypeInput_Button) {
      hidDeviceRecord->buttonElements = realloc(
          hidDeviceRecord->buttonElements,
          sizeof(struct HIDGamepadButton) * (deviceRecord->numButtons + 1));
      hidDeviceRecord->buttonElements[deviceRecord->numButtons].cookie =
          IOHIDElementGetCookie(element);
  ...
  IOHIDDeviceRegisterInputValueCallback(device, onDeviceValueChanged,
                                        deviceRecord);
  ...
  if (deviceEventCount >= deviceEventQueueSize) {
    deviceEventQueueSize =
        deviceEventQueueSize == 0 ? 1 : deviceEventQueueSize * 2;
    deviceEventQueue =
        realloc(deviceEventQueue,
                sizeof(struct Gamepad_queuedEvent) * deviceEventQueueSize);
  }
...
static void disposeDevice(struct Gamepad_device *deviceRecord) {
  ...
  IOHIDDeviceRegisterInputValueCallback(
      ((struct Gamepad_devicePrivate *)deviceRecord->privateData)->deviceRef,
      NULL, NULL);

  for (inputEventIndex = 0; inputEventIndex < inputEventCount;
       inputEventIndex++) {
    if (inputEventQueue[inputEventIndex].deviceID == deviceRecord->deviceID) {
      ...
      inputEventCount--;
      for (inputEventIndex2 = inputEventIndex;
           inputEventIndex2 < inputEventCount; inputEventIndex2++) {
        inputEventQueue[inputEventIndex2] =
            inputEventQueue[inputEventIndex2 + 1];
      }
      inputEventIndex--;
    }
  }

  for (deviceEventIndex = 0; deviceEventIndex < deviceEventCount;
       deviceEventIndex++) {
    if (deviceEventQueue[deviceEventIndex].deviceID == deviceRecord->deviceID) {
      unsigned int deviceEventIndex2;

      deviceEventCount--;
      for (deviceEventIndex2 = deviceEventIndex;
           deviceEventIndex2 < deviceEventCount; deviceEventIndex2++) {
        deviceEventQueue[deviceEventIndex2] =
            deviceEventQueue[deviceEventIndex2 + 1];
      }
      deviceEventIndex--;
    }
  }

  free(((struct Gamepad_devicePrivate *)deviceRecord->privateData)
           ->axisElements);
  free(((struct Gamepad_devicePrivate *)deviceRecord->privateData)
           ->buttonElements);
...
static void onDeviceRemoved(void *context, IOReturn result, void *sender,
                            IOHIDDeviceRef device) {
  unsigned int deviceIndex;

  for (deviceIndex = 0; deviceIndex < numDevices; deviceIndex++) {
    if (((struct Gamepad_devicePrivate *)devices[deviceIndex]->privateData)
            ->deviceRef == device) {
...
void Gamepad_init() {
  if (hidManager == NULL) {
    ...
    values[1] =
        CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &value);
    dictionaries[0] = CFDictionaryCreate(
    ...
    values[1] =
        CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &value);
    dictionaries[1] = CFDictionaryCreate(
        kCFAllocatorDefault, (const void **)keys, (const void **)values, 2,
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
    CFRelease(values[0]);
    CFRelease(values[1]);
    ...
    values[1] =
        CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &value);
    dictionaries[2] = CFDictionaryCreate(
        kCFAllocatorDefault, (const void **)keys, (const void **)values, 2,
        &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
    CFRelease(values[0]);
    CFRelease(values[1]);

    array = CFArrayCreate(kCFAllocatorDefault, (const void **)dictionaries, 3,
    ...
    IOHIDManagerRegisterDeviceMatchingCallback(hidManager, onDeviceMatched,
                                               NULL);
    IOHIDManagerRegisterDeviceRemovalCallback(hidManager, onDeviceRemoved,
                                              NULL);

    IOHIDManagerOpen(hidManager, kIOHIDOptionsTypeNone);

    // Force gamepads to be recognized immediately. The normal run loop mode
    // takes a few frames, but we can run one iteration with a custom mode to do
    // it without a delay.
    IOHIDManagerScheduleWithRunLoop(hidManager, CFRunLoopGetCurrent(),
                                    GAMEPAD_RUN_LOOP_MODE);
...
unsigned int Gamepad_numDevices() { return numDevices; }
...
static void processQueuedEvent(struct Gamepad_queuedEvent event) {
  ...
  case GAMEPAD_EVENT_BUTTON_DOWN:
    if (Gamepad_buttonDownCallback != NULL) {
      ...
      Gamepad_buttonDownCallback(buttonEvent->device, buttonEvent->buttonID,
  ...
  case GAMEPAD_EVENT_BUTTON_UP:
    if (Gamepad_buttonUpCallback != NULL) {
      struct Gamepad_buttonEvent *buttonEvent = event.eventData;
      Gamepad_buttonUpCallback(buttonEvent->device, buttonEvent->buttonID,
                               buttonEvent->timestamp, Gamepad_buttonUpContext);
    }
    break;
  ...
  case GAMEPAD_EVENT_AXIS_MOVED:
    if (Gamepad_axisMoveCallback != NULL) {
      ...
      Gamepad_axisMoveCallback(axisEvent->device, axisEvent->axisID,
                               axisEvent->value, axisEvent->lastValue,
                               axisEvent->timestamp, Gamepad_axisMoveContext);
    }
    break;
  }
}
